# -*- coding: utf-8 -*-
"""
classifier_api_github.py — Bộ phân loại AI chuyên dụng (Thuần AI) cho GITHUB ACTIONS
=============================================================================
Được tối ưu hóa độc quyền để chạy trong GitHub Actions Workflow.
Kết nối duy nhất tới CSDL PostgreSQL đám mây thông qua db_adapter_github.py.
Sử dụng 100% mô hình AI (Gemma/Gemini) trực tuyến thông qua Google AI Studio API.
Loại bỏ hoàn toàn các thư viện Offline ML (.pkl) cồng kềnh.
=============================================================================
"""
...
# Nạp file .env nếu chạy local để hỗ trợ debug kết nối PostgreSQL đám mây
try:
...
# Import các hàm nghiệp vụ CSDL chuyên dụng cho GitHub Actions
from crawler_db_github import init_db, get_unclassified_articles, save_classified_articles_batch
...
def call_gemini_with_fallback(prompt, chunk):
    ...
    if not api_key:
        raise Exception("❌ Không tìm thấy GEMINI_API_KEY trong biến môi trường GitHub Secrets.")
...
Những bài này CẦN PHẢI được nhận diện nhạy bén, đánh giá kỹ để gán is_matched: true and domain_id phù hợp.
...
def main():
    print("🧠 BAT DAU PIPELINE PHAN LOAI THUAN AI CHUYEN DUNG CHO GITHUB ACTIONS 🧠\n")
    
    # 0. Khởi tạo cơ sở dữ liệu nếu chưa tồn tại bảng
    init_db()
    
    # 1. Đọc danh sách bài viết chưa được phân loại từ DB PostgreSQL đám mây
    unclassified = get_unclassified_articles()
    total_unclassified = len(unclassified)
    print(f"📂 Tim thay {total_unclassified} bai viet tho chua duoc xu ly phan loai.")
    
    if total_unclassified == 0:
        print("✨ Tuyet voi! Toan bo bai viet trong DB da duoc phan loai.")
        return

    # 2. Chuẩn bị 100% dữ liệu để đẩy lên AI
    print(f"🔍 Dang chuan bi {total_unclassified} bai viet de gui truc tiep len Google AI Studio...")
    suspect_articles = unclassified
    
    # 3. Phân nhóm và gửi lên Gemini song song đa luồng (12 luồng)
    chunk_size = 50
    chunks = [suspect_articles[i:i + chunk_size] for i in range(0, len(suspect_articles), chunk_size)]
    print(f"\n⚡ Dang gui {len(suspect_articles)} bai viet len AI rasoat (chia lam {len(chunks)} lo x {chunk_size} bai)...")

    api_key = os.getenv("GEMINI_API_KEY")
    if not api_key:
        print("❌ [LOI CHI TIET] Khong tim thay GEMINI_API_KEY trong GitHub Secrets. Dung tien trinh.")
        sys.exit(1)
        
    primary_model = os.getenv("GEMINI_MODEL", "gemma-4-31b-it")
    print(f"🤖 MO HINH SU DUNG: {primary_model} (kèm mô hình dự phòng gemma-4-26b-a4b-it)")
    ...
    with ThreadPoolExecutor(max_workers=12) as executor:
        ...
        for idx, chk in enumerate(chunks):
            # Tạo khoảng trễ nhỏ rải đều các thread
            delay = (idx % 12) * 1.5
        ...
        for future in as_completed(futures):
            try:
                ...
                print(f"❌ Loi xu ly luong AI: {e}")
                
    # 4. Đối chiếu dữ liệu đầu ra và chuẩn bị lưu trữ
    ai_matched_count = 0
    records_to_save = []
    
    print("\n🖥️  KET QUA PHAN TICH CHI TIET:")
    for res in final_results:
        ...
        c_model = res.get("classifier_model", primary_model)
        ...
        if is_matched and c_status != "LOCATION_REJECTED":
            print(f"✅ [KHOP] Linh vuc {dom_id} | {title[:65]}... (Score: {score:.2f}) | {reason}")
        elif c_status == "LOCATION_REJECTED":
            print(f"❌ [LOAI DIA BAN] {title[:65]}... | {reason}")
    ...
    # 5. Lưu hàng loạt kết quả vào database PostgreSQL đám mây thông qua Batch Transaction
    if records_to_save:
        print(f"\n⚡ Dang dong bo gui hang loat {len(records_to_save)} bai viet da phan loai len Postgres Cloud...")
        saved_count = save_classified_articles_batch(records_to_save)
        print(f"🎉 Dong bo hoan tat! Da cap nhat {saved_count} bai viet trong CSDL.")
        
    # 6. Tự động loại bỏ hoàn toàn các tin ngoài địa bàn Hà Nội trực tiếp trong PostgreSQL
    try:
        ...
        conn.close()
        if rejected_count > 0:
            print(f"Sweep: Tu dong loai bo và chuyen {rejected_count} bai ngoai dia ban Ha Noi sang tab 'Bi loai bo'.")
    except Exception as e:
        print(f"⚠️ Loi khi quet loai bo tin dia ban: {e}")
        
    print(f"\n🎉 HOAN THANH PIPELINE CLASSIFIER:")
    print(f"   - Tong so tin gui len AI kiem duyet: {len(suspect_articles)}")
    print(f"   - Tong so tin AI xac dinh vi pham tai Ha Noi: {ai_matched_count}")
    
    # 7. Xuất báo cáo thống kê trực quan từ PostgreSQL Cloud
    print("\n================ BAO CAO THONG KE LINH VUC BAO VE (PostgreSQL Cloud) ================")
    try:
        conn = db_adapter_github.get_conn()
        cursor = conn.cursor()
        cursor.execute('''
        ...
        if stats:
            ...
            print("☘️ Chua phat hien tin bai vi pham/can bao ve nao.")
        conn.close()
    except Exception as e:
        print(f"⚠️ Loi lay thong ke tu CSDL: {e}")
    print("==================================================================================\n")
